function makeBackground() {
  ...
  if (bgCol < 8 || packType==1){
    ...
    background(190);
  } else if (bgCol < 9.3 || addTree == true) {
    background(100);
  } else {
    background(0);
  }
...
function colorBG() {
  ...
  if (random(2) < 1 && addTree == false) {
    colBG = color(huey, 90, 20, 255);
  } else {
    colBG = color(huey, sat, 50, 255);
  }
}
...
function getColor() {
  ...
  noiseChance = random(55, 70); //chance out of 100 that noise will determine color; otherwise random
